...
const renameCommandName = "multi-cursor-enhancer.multi-rename";
const renameActive = "multiRename.isActive";
...
export class ChangeTracker {
	...
	constructor() {
		this.dispose.track([
			...
			commands.registerCommand(renameCommandName, () =>
	...
	private async renameModifiedIdentifiers(): Promise<void> {
		...
		try {
			...
			const renames = this.tracker.trackedSpans
				.filter((trackedSpan) => trackedSpan.modified)
				.map(
					(trackedSpan) =>
						({
							uri: doc.uri,
	...
	private updateUI(): void {
		...
		if (this.tracker && this.tracker.trackedSpans.some((t) => t.modified)) {
			setContext(renameActive, true);
			this.statusBarItem.command = renameCommandName;
			this.statusBarItem.text = `$(edit) Rename ${
				this.tracker.trackedSpans.filter((t) => t.modified).length
			} changes`;
			this.statusBarItem.show();
		} else {
			setContext(renameActive, false);
